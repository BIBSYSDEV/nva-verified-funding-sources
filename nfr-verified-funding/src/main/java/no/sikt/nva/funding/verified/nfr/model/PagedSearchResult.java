...
import static no.sikt.nva.funding.verified.nfr.model.PagedSearchResult.CONTEXT_FIELD_NAME;
import static no.sikt.nva.funding.verified.nfr.model.PagedSearchResult.HITS_FIELD_NAME;
import static no.sikt.nva.funding.verified.nfr.model.PagedSearchResult.ID_FIELD_NAME;
import static no.sikt.nva.funding.verified.nfr.model.PagedSearchResult.NEXT_RESULTS_FIELD_NAME;
import static no.sikt.nva.funding.verified.nfr.model.PagedSearchResult.PREVIOUS_RESULTS_FIELD_NAME;
import static no.sikt.nva.funding.verified.nfr.model.PagedSearchResult.SIZE_FIELD_NAME;
...
@JsonPropertyOrder({
  ...
  ID_FIELD_NAME,
  SIZE_FIELD_NAME,
  NEXT_RESULTS_FIELD_NAME,
  PREVIOUS_RESULTS_FIELD_NAME,
  HITS_FIELD_NAME
})
public class PagedSearchResult<T> {
  ...
  public PagedSearchResult(
      URI context, URI baseUri, int queryOffset, int querySize, int totalSize, List<T> hits) {
    ...
    this.id =
        UriWrapper.fromUri(baseUri)
            ...
            .getUri();
    this.nextResults =
        calculateNextResults(queryOffset, querySize, totalSize, hits.size(), baseUri);
  ...
  @JacocoGenerated
  public PagedSearchResult(
      @JsonProperty(CONTEXT_FIELD_NAME) URI context,
  ...
  private static URI calculateNextResults(
      int queryOffset, int querySize, int totalSize, int noHits, URI baseUri) {
    return (queryOffset + noHits) < totalSize
        ? generateSelfUri(baseUri, queryOffset + querySize, querySize)
        : null;
  }

  @SuppressWarnings("PMD.OnlyOneReturn")
  private static URI calculatePreviousResults(int queryOffset, int querySize, URI baseUri) {
    if (queryOffset > 0) {
