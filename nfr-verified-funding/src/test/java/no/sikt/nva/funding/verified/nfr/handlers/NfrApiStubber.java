...
public class NfrApiStubber {
  ...
  public int byProjectIdSingleMatch(int totalHits) {
    ...
    var responseObject =
        new NfrFundingSearchResult(
            totalHits, 0, 10, generateSingleHitMultipleCandidates(projectId, totalHits - 1));
            ...
            () ->
                stubFor(
                    get(url)
                        .willReturn(
                            aResponse()
                                .withHeader(CONTENT_TYPE, APPLICATION_JSON_CHARSET_UTF_8)
  ...
  public int byProjectIdNoExactMatch(int totalHits) {
    ...
    var responseObject =
        new NfrFundingSearchResult(
            totalHits, 0, 10, generateNoHitsMultipleCandidates(projectId, totalHits));
  ...
  public String withMatchingLeadNameAsMostRelevantHit(
      int numberOfMatchesByName, int numberOfAdditionalMatches, int from, int size) {
    ...
    var url =
        "/search?query="
            + URLEncoder.encode(leadName, StandardCharsets.UTF_8)
                .replace("+", "%20")
                .replace("&", "%26")
                .replace(",", "%2C")
            + "&from="
  ...
  private List<NfrFunding> randomFundingsWithDifferentLeadName(
      String leadName, int numberOfFundings) {
    var fundings = new ArrayList<NfrFunding>(numberOfFundings);
    for (int counter = 0; counter < numberOfFundings; counter++) {
