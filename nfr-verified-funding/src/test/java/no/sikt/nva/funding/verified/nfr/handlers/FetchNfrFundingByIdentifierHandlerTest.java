...
class FetchNfrFundingByIdentifierHandlerTest {
  ...
  void shouldReturnOnlyOneMatchWhenQueryReturnsMultipleButOnlyOneHasExactMatchOnProjectId()
    ...
    assertThat(
        funding.getLabels().get("en"), is(equalTo(nfrFunding.getEnglishMetadata().get("title"))));
    assertThat(
        funding.getLabels().get("nb"), is(equalTo(nfrFunding.getNorwegianMetadata().get("title"))));
  ...
  @Test
  void shouldReturnNotFoundStatusCodeWhenNotFoundButCandidatesArePresent() throws IOException {
    ...
    assertThat(
        problem.getDetail(),
        is(equalTo(String.format("Funding with identifier %d not found!", projectId))));
  }
  ...
  @Test
  void shouldReturnBadGatewayOnNonSuccessStatusCodeFromNfrApi() throws IOException {
  ...
  void shouldReturnBadGatewayWhenUnableToConnectToNfrApi(WireMockRuntimeInfo runtimeInfo)
    ...
    var apiClient =
        new NfrApiClient(
            httpClient, URI.create("https://localhost:" + (runtimeInfo.getHttpsPort() - 1)));
    handlerUnderTest = new FetchNfrFundingByIdentifierHandler(environment, apiClient);
  ...
  private void shouldReturnBadGatewayWithProblemDetail(int projectId, String detail)
      throws IOException {
    var input =
        new HandlerRequestBuilder<Void>(dtoObjectMapper)
            .withPathParameters(Map.of(IDENTIFIER, Integer.toString(projectId)))
